...
class SMB():
    ...
    def run(self):
        ...
        for _ in range(self.problem.numberOfObjectives):
            SMs.append(RandomForestRegressor(n_estimators=200, max_depth=None, min_samples_split=2, random_state=0, criterion='friedman_mse', n_jobs=1, warm_start=True))

        eval = self.sample_size
        while (eval <= self.t_parada):
            Pk = list(set(Pk + Pt))

            if eval <= self.t_max or len(Archive) >= self.t_max:
                if len(Archive) >= self.t_max:
                    Archive += Pk
