...
from src.problems.factory import get_problem
from src.problems.rp import get_reference_points
from src.problems.metrics import get_igd

from src.tdeadp.evolution.initialization import lhs_init_population
from src.tdeadp.evolution.variation import random_genetic_variation
from src.tdeadp.evolution.norm import var_normalization
from src.tdeadp.evolution.scalar import cluster_scalarization
from src.tdeadp.evolution.selection import sel_scalar_dea
from src.tdeadp.evolution.algorithms import scalar_dom_ea_dp
from src.tdeadp.evolution.counter import PerCounter
from src.tdeadp.evolution.selection import pareto_scalar_nn_filter
from src.tdeadp.evolution.ranking import non_dominated_ranking
from src.tdeadp.evolution.dom import pareto_dominance

from src.tdeadp.learning.model_init import init_dom_nn_classifier
from src.tdeadp.learning.model_update import update_dom_nn_classifier


from run_hyper import save, get_problem as get_problem_svm

problem = get_problem_svm("SVM_hyperparameters_breast", "resources/args_samples/M1_25_1000.json")
...
#problem = get_problem('dtlz4', n_var=10, n_obj=8)

#pf_path = "resources/ReferenceFronts/pf/DTLZ1.2D.pf"     # the path to true Pareto front of the problem
#pf_path = "pf/ZDT1.2D.pf"
#pf_path = "pf/ZDT2.2D.pf"
#pf_path = 'pf5/dtlz4_8objs.txt'

ref_points = get_reference_points(problem.numberOfObjectives)  # define a set of structured weight vectors

MU = len(ref_points)  # population size
INIT_SIZE = 100 #11 * problem.n_var - 1  # the number of initial solutions

MAX_EVALUATIONS = 500   # The maximum number of function evaluations, should be larger than INIT_SIZE


LAMBDA = 50    # the number of offsprings
CXPB = 0.9       # crossover probability
MUTPB = 0.5      # mutation probability

DIC = 20                    # distribution index for crossover
...
WINDOW_SIZE = 11 * problem.numberOfDecisionVariables + 24 #problem.n_var + 24      # the maximum number of solutions used in updating

CATEGORY_SIZE = 300                        # the maximum size in each category

# create types for the problem, refer to DEAP documentation
creator.create("FitnessMin", base.Fitness, weights=(-1.0,) * problem.numberOfObjectives)
creator.create("Individual", list, fitness=creator.FitnessMin)

toolbox = base.Toolbox()

# customize the population initialization
toolbox.register("population", lhs_init_population, list, creator.Individual, problem.decisionVariablesLimit[0], problem.decisionVariablesLimit[1])
...
toolbox.register("mate", tools.cxSimulatedBinaryBounded, low=list(problem.decisionVariablesLimit[0]), up=list(problem.decisionVariablesLimit[1]), eta=30.0)

# customize the mutation operator, polynomial mutation is used here
toolbox.register("mutate", tools.mutPolynomialBounded, low=list(problem.decisionVariablesLimit[0]), up=list(problem.decisionVariablesLimit[1]), eta=20.0,
                 indpb=1.0 / problem.numberOfDecisionVariables)
...
toolbox.register("normalize_variables", var_normalization, low=problem.decisionVariablesLimit[0], up=problem.decisionVariablesLimit[1])
...
toolbox.register("init_pareto_model", init_dom_nn_classifier,
                 device=device,
                 input_size=2 * problem.numberOfDecisionVariables, hidden_size=HIDDEN_SIZE,
...
toolbox.register("filter", pareto_scalar_nn_filter, device=device, ref_points=ref_points,
                 counter=PerCounter(len(ref_points)), toolbox=toolbox, visualization=False)
...
#np.savetxt(out_path, np.array(Y))
    save("TDEAP_5000_teste2", str(i), X, Y, T)
    print("*" * 80)
